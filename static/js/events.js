...
async function updateGrid() {
  ...
  eventGrid.innerHTML = events.map((event) => `
      <a class="card" href="/${window.currentLang}/events/${event.slug}">
        <img src="${event.images[0]}" alt="${event.title_text}" />
        <div class="card-body"><span class="badge">${event.category}</span><h3>${event.title_text}</h3><p>${event.date} · ${event.time}</p></div>
      </a>`).join('');
}
